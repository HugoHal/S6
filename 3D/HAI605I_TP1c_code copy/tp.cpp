...
void setUnitSphere(Mesh &o_mesh, int nX = 20, int nY = 20)
{
  ...
  { 
    ...
    o_mesh.triangles.push_back(Triangle((nX) + i + 1, (nX) + i, (nX + 1)));
  }
  Vec3 vertex(0, 0, 0);
  o_mesh.vertices.push_back(vertex);
  o_mesh.normals.push_back(vertex);
  ...
  {
    if (i != nX) {
    o_mesh.triangles.push_back(Triangle(i, nX*2+2, i + 1));}
    o_mesh.triangles.push_back(Triangle(nX + i + 2 , nX*2+2, nX + i + 1));
  } 
             ...
             bool save_normals = true)
{
  ...
  {
    myfile << 3 << " " << i_triangles[f][0] << " " << i_triangles[f][1] << " "
           << i_triangles[f][2];
    myfile << std::endl;
  }
  myfile.close();
